...
import { STEP_META } from "@/components/automations/automation-builder"
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setError(err instanceof Error ? err.message : t('automations.logs.errorLoad'))
      }
    }
    load()
  }, [id, t])
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <button
          ...
          aria-label={t('broadcasts.wizard.back')}
        >
          <ArrowLeft className="h-4 w-4" />
        </button>
        <div>
          <h1 className="text-2xl font-bold text-foreground">{automation.name}</h1>
          <p className="mt-0.5 text-sm text-muted-foreground">{t('automations.logs.subtitle')}</p>
        </div>
      </div>

      {logs.length === 0 ? (
        <div className="flex h-48 flex-col items-center justify-center rounded-xl border border-dashed border-border bg-card/40">
          <p className="text-sm text-foreground">{t('automations.logs.emptyTitle')}</p>
          <p className="mt-1 text-xs text-muted-foreground">
            {t('automations.logs.emptySubtitle')}
          </p>
        </div>
      ) : (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="truncate text-sm font-medium text-foreground">
                      {log.contact?.name ?? log.contact?.phone ?? t('automations.logs.unknownContact')}
                    </div>
                    <div className="truncate text-xs text-muted-foreground">
                      {log.trigger_event} ·{" "}
                      {t(
                        (log.steps_executed?.length ?? 0) === 1
                          ? 'automations.logs.stepCount'
                          : 'automations.logs.stepCountPlural',
                        { count: log.steps_executed?.length ?? 0 },
                      )}
                    </div>
                  </div>
                  <div className="text-xs text-muted-foreground">
                    {formatRelative(log.created_at, t, locale)}
                  </div>
                </button>
                {isOpen && (
...
function StatusBadge({ status }: { status: AutomationLog["status"] }) {
  ...
  const label =
    status === "success"
      ? t('automations.logs.statusSuccess')
      : status === "partial"
      ? t('automations.logs.statusPartial')
      : t('automations.logs.statusFailed')
  return (
    <span
      className={cn(
...
function StepRow({ result }: { result: AutomationLogStepResult }) {
  ...
  return (
    <li className="flex items-start gap-2 text-xs">
      ...
      <span className="text-muted-foreground">{meta ? t(meta.labelKey) : result.step_type}</span>
      {result.detail && (
