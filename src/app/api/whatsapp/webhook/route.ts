...
// flagBroadcastReplyIfAny moved to src/lib/whatsapp/inbound-message.ts
// (part of the provider-agnostic tail shared with the Evolution webhook).
...
async function processMessage(
  ...
  accessToken: string,
  // Id of the matched whatsapp_config row — binds a brand-new
  // conversation to this channel (043_whatsapp_multi_channel.sql).
  configId: string,
) {
  ...
  // Everything past this point (insert into `messages`, update
  // `conversations`, broadcast-reply flag, flow/automation dispatch)
  // is provider-agnostic — shared with the Evolution API webhook. See
...
// findOrCreateContact / findOrCreateConversation moved to
// src/lib/whatsapp/inbound-message.ts (shared with the Evolution webhook).
