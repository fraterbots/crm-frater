...
export function Step1ChooseTemplate({ selectedTemplate, onSelect, onNext, onBack }: Step1Props) {
  ...
  useEffect(() => {
    async function fetchTemplates() {
      try {
        ...
        setError(err instanceof Error ? err.message : t('broadcasts.step1.errorLoad'));
      } finally {
        setLoading(false);
      }
    }
    ...
    fetchTemplates();
    // `t` is intentionally omitted — this effect fetches once on mount;
    // re-running the fetch on a locale switch would be wasteful.
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold text-foreground">{t('broadcasts.step1.title')}</h2>
        <p className="mt-1 text-sm text-muted-foreground">
          {t('broadcasts.step1.subtitle')}
        </p>
      </div>
      ...
      {templates.length === 0 ? (
        <div className="flex h-48 flex-col items-center justify-center rounded-xl border border-border bg-card/50">
          ...
          <p className="text-sm text-muted-foreground">{t('broadcasts.step1.emptyTitle')}</p>
          <p className="mt-1 text-xs text-muted-foreground">{t('broadcasts.step1.emptySubtitle')}</p>
        </div>
      ) : (
