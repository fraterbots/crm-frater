  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="border-border bg-popover text-popover-foreground sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-popover-foreground">{t('contacts.customFields.title')}</DialogTitle>
          <DialogDescription className="text-muted-foreground">
            {t('contacts.customFields.description')}
          </DialogDescription>
        </DialogHeader>
        <CustomFieldsPanel />
      </DialogContent>
...
export function CustomFieldsPanel() {
  ...
  async function handleCreate() {
    ...
    if (!name) return;
    if (!accountId || !user) {
      toast.error(t('contacts.customFields.errorNoAccount'));
      return;
    }
    if (isDuplicate(name)) {
      toast.error(t('contacts.customFields.errorDuplicate', { name }));
      return;
    }
    ...
    if (error) {
      toast.error(t('contacts.customFields.errorCreate'));
      return;
    }
    toast.success(t('contacts.customFields.created', { name }));
    setNewName('');
    await fetchFields();
  }
  ...
  async function handleRename(
    ...
    setBusyId(null);
    if (error) {
      toast.error(t('contacts.customFields.errorRename'));
      return false;
    }
    await fetchFields();
    return true;
  ...
  async function handleDelete(field: CustomField) {
    if (
      !window.confirm(
        t('contacts.customFields.confirmDelete', { name: field.field_name }),
      )
    ) {
      return;
    }
    ...
    setBusyId(null);
    if (error) {
      toast.error(t('contacts.customFields.errorDelete'));
      return;
    }
    toast.success(t('contacts.customFields.deleted', { name: field.field_name }));
    await fetchFields();
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        <Input
          ...
          placeholder={t('contacts.customFields.newFieldPlaceholder')}
      ...
      <div className="max-h-72 overflow-y-auto rounded-md border border-border">
        {loading ? (
          ...
          <p className="py-8 text-center text-sm text-muted-foreground">
            {t('contacts.customFields.noFieldsYet')}
          </p>
        ) : (
  ...
  return (
    <li className="flex items-center gap-2 px-3 py-2">
      <Input
        ...
        aria-label={t('contacts.customFields.renameAria', { name: field.field_name })}
