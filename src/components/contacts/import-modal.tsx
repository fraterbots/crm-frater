      ...
      {tagNames.map((name) => {
        ...
        return (
          <span
            ...
            title={isKnown ? name : t('contacts.import.willBeCreated', { name })}
          >
            <span
...
export function ImportModal({
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (rows.length === 0) {
      toast.error(t('contacts.import.errorNoRows'));
      setParsedRows([]);
      setHasTagsColumn(false);
      setHasCompanyColumn(false);
      setTagColorByKey(new Map());
  ...
  async function handleImport() {
    ...
    try {
      ...
      if (!user) throw new Error(t('contacts.import.errorNotAuthenticated'));
      if (!accountId)
        throw new Error(t('contacts.import.errorNoAccount'));
      ...
      try {
        ...
        toast.warning(t('contacts.import.warnTagAssignFailed'));
      }

      setResult({ imported, skipped, failed, tagsAssigned });
      if (imported > 0) {
        toast.success(
          t(imported === 1 ? 'contacts.import.toastImported' : 'contacts.import.toastImportedPlural', {
            count: imported,
          }),
        );
        onImported();
      ...
      if (tagsAssigned > 0) {
        toast.success(
          t(
            tagsAssigned === 1
              ? 'contacts.import.toastTagAssignments'
              : 'contacts.import.toastTagAssignmentsPlural',
            { count: tagsAssigned },
          ),
        );
      }
      if (skippedNames.length > 0) {
        ...
        const more =
          skippedNames.length > 3
            ? t('contacts.import.toastUnknownTagsMore', { count: skippedNames.length - 3 })
            : '';
        toast.info(t('contacts.import.toastUnknownTags', { sample: `${sample}${more}` }));
      }
      if (skipped > 0) {
        toast.info(
          t(
            skipped === 1 ? 'contacts.import.toastDuplicatesSkipped' : 'contacts.import.toastDuplicatesSkippedPlural',
            { count: skipped },
          ),
        );
      }
      if (failed > 0) {
        toast.error(
          t(failed === 1 ? 'contacts.import.toastFailed' : 'contacts.import.toastFailedPlural', {
            count: failed,
          }),
        );
      }
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : t('contacts.import.errorGeneric');
      toast.error(message);
    } finally {
      setImporting(false);
    }
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="flex max-h-[min(90vh,720px)] flex-col gap-0 overflow-hidden border-border/80 bg-popover p-0 text-popover-foreground sm:max-w-2xl">
        <div className="shrink-0 space-y-4 border-b border-border/80 px-6 pt-6 pb-5">
          <DialogHeader className="gap-1.5">
            ...
            <DialogDescription className="leading-relaxed text-muted-foreground">
              ...
              {t('contacts.import.descTagsHint')}
            </DialogDescription>
          </DialogHeader>
            ...
            {file ? (
              <>
                ...
                <span className="rounded-full bg-muted px-2.5 py-0.5 text-[11px] font-medium text-muted-foreground">
                  {t(
                    parsedRows.length === 1 ? 'contacts.import.rowsReady' : 'contacts.import.rowsReadyPlural',
              ...
              <>
                ...
                <p className="text-sm text-muted-foreground">
                  {t('contacts.import.dropZoneCTA')}
                </p>
                <p className="text-[11px] text-muted-foreground">
                  {t('contacts.import.dropZoneHint')}
                </p>
              </>
            )}
          </div>
                ...
                <div className="flex flex-wrap items-center gap-1.5">
                  {tagStats.rowsWithTags > 0 && (
                    <span className="inline-flex items-center gap-1 rounded-md bg-muted/90 px-2 py-0.5 text-[11px] text-muted-foreground">
                      ...
                      {t(
                        tagStats.unique === 1 ? 'contacts.import.tagCount' : 'contacts.import.tagCountPlural',
                        { count: tagStats.unique },
                      )}{' '}
                      ·{' '}
                      {t(
                        tagStats.rowsWithTags === 1
                          ? 'contacts.import.contactCount'
                          : 'contacts.import.contactCountPlural',
                        { count: tagStats.rowsWithTags },
                      )}
                    </span>
                  )}
                </div>
                    ...
                    <thead>
                      <tr className="border-b border-border bg-background/60">
                        <th className="px-3 py-2 text-left font-medium whitespace-nowrap text-muted-foreground">
                          {t('contacts.page.colPhone')}
                        </th>
                        <th className="px-3 py-2 text-left font-medium whitespace-nowrap text-muted-foreground">
                          {t('contacts.page.colName')}
                        </th>
                        <th className="px-3 py-2 text-left font-medium whitespace-nowrap text-muted-foreground">
                          {t('contacts.page.colEmail')}
                        </th>
                        {previewHasCompany && (
                          <th className="px-3 py-2 text-left font-medium whitespace-nowrap text-muted-foreground">
                            {t('contacts.page.colCompany')}
                          </th>
                        )}
                        {previewHasTags && (
                          <th className="px-3 py-2 text-left font-medium whitespace-nowrap text-muted-foreground">
                            {t('contacts.page.colTags')}
                          </th>
                        )}
                      </tr>
                    </thead>
                ...
                <p className="text-center text-[11px] text-muted-foreground">
                  {t(
                    parsedRows.length - PREVIEW_LIMIT === 1
                      ? 'contacts.import.moreRowsNotShown'
                      : 'contacts.import.moreRowsNotShownPlural',
              ...
              <div className="mt-3 flex flex-wrap gap-3">
                {result.imported > 0 && (
                  <div className="text-primary flex items-center gap-1.5 text-sm">
                    ...
                    {t('contacts.import.imported', { count: result.imported })}
                  </div>
                )}
                  ...
                  <div className="flex items-center gap-1.5 text-sm text-cyan-400">
                    ...
                    {t(
                      result.tagsAssigned === 1
                        ? 'contacts.import.tagsAssigned'
                        : 'contacts.import.tagsAssignedPlural',
                      { count: result.tagsAssigned },
                    )}
                  </div>
                )}
                {result.skipped > 0 && (
                ...
                {result.failed > 0 && (
                  <div className="flex items-center gap-1.5 text-sm text-red-400">
                    ...
                    {t('contacts.import.failed', { count: result.failed })}
                  </div>
                )}
              </div>
            </div>
        ...
        <DialogFooter className="mt-0 shrink-0 gap-2 border-t border-border/80 bg-background/50 px-6 py-4 sm:justify-end">
          ...
          >
            {result ? t('contacts.import.close') : t('contacts.import.cancel')}
          </Button>
          {!result && (
            <Button
              type="button"
            ...
            >
              ...
              {t(
                parsedRows.length === 1 ? 'contacts.import.importButton' : 'contacts.import.importButtonPlural',
                { count: parsedRows.length },
              )}
            </Button>
          )}
