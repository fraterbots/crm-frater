      ...
      <header className="flex items-center justify-between border-b border-border px-5 py-4">
        ...
        <div className="flex items-center gap-1 rounded-lg bg-muted/60 p-1">
          {[7, 30, 90].map((r) => (
            ...
            >
              {t('dashboard.conversationsChart.rangeDays', { count: r })}
            </button>
          ))}
        </div>
      </header>
      ...
      <div className="p-5">
        {loading || !data ? (
          ...
          <LineSvg
            ...
            maxY={maxY}
            ticks={niceTicks}
            ariaLabel={t('dashboard.conversationsChart.ariaLabel')}
          />
        )}
      </div>

      <footer className="flex items-center gap-4 border-t border-border px-5 py-3 text-xs text-muted-foreground">
        <LegendDot color="#3b82f6" label={t('dashboard.conversationsChart.legendIncoming')} />
        <LegendDot color="#7c3aed" label={t('dashboard.conversationsChart.legendOutgoing')} />
      </footer>
    </section>
  )
}
