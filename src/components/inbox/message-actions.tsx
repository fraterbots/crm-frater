...
export function MessageActions({
  ...
  const handleCopy = async () => {
    ...
    if (!text) {
      toast.error(t("inbox.messageActions.nothingToCopy"));
      return;
    }
    try {
      await navigator.clipboard.writeText(text);
      toast.success(t("inbox.messageActions.copied"));
    } catch {
      toast.error(t("inbox.messageActions.copyFailed"));
    }
    setTouchOpen(false);
  };
      ...
      >
        <Popover open={pickerOpen} onOpenChange={setPickerOpen}>
          <PopoverTrigger
            ...
            aria-label={t("inbox.messageActions.reactAria")}
          >
          ...
          >
            {QUICK_EMOJIS.map((e) => (
              <button
                ...
                aria-label={t("inbox.messageActions.reactWithAria", { emoji: e })}
              >
                {e}
              </button>
            ))}
        ...
        <button
          ...
          onClick={handleReply}
          className="flex h-5 w-5 items-center justify-center rounded-full text-popover-foreground hover:bg-muted hover:text-foreground"
          aria-label={t("inbox.messageActions.replyAria")}
        >
        ...
        <button
          ...
          onClick={handleCopy}
          className="flex h-5 w-5 items-center justify-center rounded-full text-popover-foreground hover:bg-muted hover:text-foreground"
          aria-label={t("inbox.messageActions.copyAria")}
        >
