...
export function PipelineSettings({
  ...
  async function handleSave() {
    ...
    if (renameRes.error || stagesRes.error) {
      toast.error(t("pipelines.settings.errorSave"));
      return;
    }
    ...
    onPipelinesChanged();
    onStagesChanged();
    toast.success(t("pipelines.settings.saved"));
  }
  ...
  async function handleAddStage() {
    ...
    const { data, error } = await supabase
      ...
      .single();
    if (error || !data) {
      toast.error(t("pipelines.settings.errorAddStage"));
      return;
    }
  ...
  async function handleRemoveStage(stageId: string) {
    ...
    const { count } = await supabase
      ...
      .eq("stage_id", stageId);
    if (count && count > 0) {
      toast.error(t("pipelines.settings.errorStageHasDeals"));
      return;
    }
    const { error } = await supabase
      ...
      .eq("id", stageId);
    if (error) {
      toast.error(t("pipelines.settings.errorDeleteStage"));
      return;
    }
  ...
  async function handleDeletePipeline() {
    ...
    setDeleting(false);
    if (error) {
      toast.error(t("pipelines.settings.errorDeletePipeline"));
      return;
    }
    onOpenChange(false);
    onPipelinesChanged();
    toast.success(t("pipelines.settings.pipelineDeleted"));
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md bg-popover border-border max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-popover-foreground">{t("pipelines.settings.manageTitle")}</DialogTitle>
        </DialogHeader>
        ...
        {showDeleteConfirm ? (
          <div className="py-4">
            <div className="flex items-center gap-3 rounded-lg border border-red-500/30 bg-red-500/10 p-4">
              ...
              <div>
                <p className="text-sm font-medium text-red-400">
                  {t("pipelines.settings.deletePipeline")}
                </p>
                <p className="mt-1 text-xs text-muted-foreground">
                  {t("pipelines.settings.deletePipelineWarningText")}
                </p>
              </div>
            </div>
            <div className="mt-4 flex justify-end gap-2">
              ...
              >
                {deleting ? t("pipelines.dealForm.deleting") : t("pipelines.settings.deletePipeline")}
              </Button>
            </div>
          </div>
        ) : (
          <>
            <div className="grid gap-4 py-2">
              <div className="grid gap-2">
                <Label className="text-muted-foreground">{t("pipelines.page.pipelineNameLabel")}</Label>
                <Input
                  value={name}
              ...
              <div className="grid gap-2">
                <Label className="text-muted-foreground">{t("pipelines.settings.stagesLabel")}</Label>
                <DndContext
                  sensors={sensors}
                ...
                <div className="mt-1 flex flex-wrap gap-1">
                  {STAGE_COLORS.map((color) => (
                    <button
                      ...
                      aria-label={t("pipelines.settings.pickColorAria", { color })}
                    />
                  ))}
                </div>
                <div className="flex items-center gap-2">
                  <Input
                    ...
                    placeholder={t("pipelines.settings.newStageNamePlaceholder")}
                  ...
                  >
                    ...
                    {t("pipelines.settings.addButton")}
                  </Button>
                </div>
              </div>
              ...
              >
                <Plus className="mr-1 h-3 w-3" />
                {t("pipelines.settings.createNewPipeline")}
              </Button>
            </div>
            ...
            <DialogFooter className="border-border bg-popover/50">
              ...
              >
                {saving ? t("pipelines.dealForm.saving") : t("pipelines.dealForm.saveChanges")}
              </Button>
            </DialogFooter>
          </>
        )}
  ...
  return (
    ...
    >
      <button
        ...
        aria-label={t("pipelines.settings.dragToReorderAria")}
      >
  ...
  return (
    <div className="relative">
      <button
        ...
        aria-label={t("pipelines.settings.changeColorAria")}
      />
      {open && (
        <>
